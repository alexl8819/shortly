                ...
                <nav className='xl:p-0 p-6 divide-y xl:divide-none divide-grayish-violet/25 flex xl:flex-row flex-col xl:justify-between justify-center items-center w-full' aria-label='main navigation'> 
                    ...
                    { 
                        links.map((link: NavLink, index: number) => (
                            <Link className='xl:mb-0 mb-[1.875rem] last:mb-0 font-bold text-[1.125rem] hover:text-very-dark-blue xl:text-grayish-violet text-white' key={index} href={link.url}>{link.label}</Link> )
                        ) 
                    }
                    </ul>
                    <div className='xl:pt-0 pt-8 xl:space-x-7 xl:w-auto flex xl:flex-row flex-col justify-center items-center text-center'>
                        <Link className='font-bold text-[1.125rem] hover:text-very-dark-blue xl:text-grayish-violet text-white' href='login'>Login</Link>
                        <Link className='xl:mt-0 mt-6 py-2 px-6 rounded-full bg-cyan hover:bg-light-cyan font-bold text-[1.125rem] text-white w-full' href='sign-up'>Sign Up</Link>
                    </div>
                </nav>
            </div>
        </div>
